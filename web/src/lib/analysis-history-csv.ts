...
import { analysisRunJoints, analysisRunKindLabel, analysisRunSourceLabel } from "./analysis-history.ts";
...
export function buildAnalysisHistoryCsv(rows: AnalysisHistoryCsvRow[]) {
  const headers = [
    ...
    "開始日時", "終了日時", "解析ステータス", "実行元", "AIモデルバージョン",
  ...
  const body = rows.map((r) => {
    ...
    return [
      ...
      SCREENING_STATUS_LABELS[r.status as ScreeningStatus] ?? r.status,
      r.source ? analysisRunSourceLabel(r.source) : null, r.ai_model_version,
