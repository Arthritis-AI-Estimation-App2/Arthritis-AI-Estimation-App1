...
判定設定の正本は `src/lib/image-quality.ts` の `IMAGE_QUALITY_CONFIG`（`hand-guide-v3`）です。評価画像は拡大せず長辺最大512pxです。手形マスクは輪郭から約8 viewBox単位以上（評価画像で最低2px）を内側へ除外し、位置合わせの小さなずれや境界の補間の影響を減らします。ファイル選択の楕円は従来どおり境界から2px内側を評価します。Laplacianは中心と上下左右がすべて評価範囲内の画素だけで計算します。
