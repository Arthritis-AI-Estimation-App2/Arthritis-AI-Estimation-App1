...
医療機関スタッフが診察室などで手指を撮影し、AI解析結果を管理するWebアプリケーションです。管理者は医療機関とスタッフを管理します。患者自身のログイン機能はなく、患者の氏名などの個人情報は保持しません。

スタッフ向けアプリ（撮影・解析・匿名IDによるグルーピング）は `/`、管理者向けアプリは `/admin` から利用します。

このディレクトリが Vercel の Root Directory です。ダッシュボードで Root Directory を `web` にしてください。リポジトリ全体の説明は [../README.md](../README.md) を参照してください。

`vercel.json` の `ignoreCommand` は、同じプロジェクト・ブランチで前回成功したデプロイのコミット（`VERCEL_GIT_PREVIOUS_SHA`）から `HEAD` までの `web/` の差分で、ビルドするかどうかを判断します。複数コミットをまとめて push した場合も、その間の変更を含めて比較します。差分がない場合だけビルドをスキップします。SHA が未設定のとき、浅いクローンで比較元を取得できないとき、Git の比較に失敗したときはビルドを実行します。
...
- 個別マイグレーションで更新する場合は、未適用のファイルを番号順に実行します（v3〜v25）。現在の最終マイグレーションは `migration_v25_account_deletion.sql` です。v25は管理者・スタッフアカウントの削除に対応するため、`profiles`から`auth.users`への外部キーのcascadeを外し、削除済みを示す`deleted_at`列を追加します。既存データは削除しません。
...
ローカルSupabaseを使う場合は `npx supabase start` のあと、`npx supabase status` の API URL・Publishable key・Secret key を設定します。旧CLIで新形式が表示されない場合はanon key・service_role keyを使います。
...
AI_API_URL=
AI_API_KEY=
AI_API_LOG_RESPONSE=false
ENABLE_DARK_MODE=false
```

ホスト済みプロジェクトを使う場合は URL を `https://<project-ref>.supabase.co` に変えてください。`ENABLE_DARK_MODE=true` にすると、OS設定に応じたダークモードが有効になります。未設定または `false` ではライトモード固定です。
...
カメラ撮影にはブラウザのカメラ権限が必要です。`getUserMedia` は通常HTTPSが必要ですが、ローカル開発では `localhost` でも利用できます。
...
クエリはページ遷移で失われるので、アプリ内のリンクから撮影画面に入り直した場合はURLを付け直してください。推論API側は受け取った画像の長辺を1024pxに合わせて1024×1024へ正規化してから関節を切り出すため、1024pxを超える解像度は判定結果には影響しません。

## AI APIについて

`AI_API_URL` が未設定の場合は、サーバー側のモック解析（左右の判定・確率・陽性関節数を返す）が動作します。

設定した場合は、Next.js Server Actionが`AI_API_URL/v1/ra-screening`へBearer認証付きで同期POSTします。`AI_API_KEY`はサーバー専用の共有APIキーであり、`NEXT_PUBLIC_`を付けないでください。リクエスト／レスポンスと画像の安全な受け渡しは[関節炎スクリーニング REST API 契約](../docs/ai_api_contract.md)と [OpenAPI](../contract/openapi.yaml) を参照してください。
...
- 所属医療機関の撮影記録一覧（被験者ID・撮影ID・日付・ステータスで絞り込み）
- 被験者IDの紐付けの訂正・解除
...
- ヘッダーの「アカウント設定」から、現在のパスワードを確認したうえで自分自身のログイン用メールアドレス・パスワードを変更
- 管理者・スタッフアカウントの削除（撮影データは削除せず、担当スタッフ名は「(削除済みユーザー)」と表示）
- 全医療機関の撮影・解析データ、撮影画像、AI判定結果の確認
- 被験者IDの紐付けの訂正・解除
- 撮影記録一覧の絞り込み条件を引き継ぐCSV出力（AI解析の集計値と左右30関節の判定・信頼度）
- 完了・失敗した記録の再解析（既存の判定結果を新しい結果で置換。モデルバージョンと解析日時を記録）
...
本番ビルドは `web/` で `npm run build` を実行します。`prebuild` が `app-version.json` にランダムなIDを生成し、Next.jsがサーバーとブラウザの両方へ埋め込みます。このファイルはGit管理外です。同じビルドを複数台へ配布する場合は成果物を共有し、各台で再ビルドしないでください。`next start` がこのIDを読むため、生成ファイルもビルド成果物と一緒に保持します。開発中は更新確認を無効にします。

ページ読み込みから1時間以上経過したあと、タブの復帰時または対象画面への移動時に、キャッシュを使わず `GET /api/version` のIDを比較します。対象はホーム（`/`）、被験者一覧（`/subjects`）、撮影記録一覧（`/screenings`、`/admin/screenings`）だけです。新しい版があるときだけページ全体を再読み込みします。入力や選択を変更した画面では離れるまで見送り、通信に失敗したときは次の確認機会まで通常どおり利用できます。表示したまま画面を移動しない場合や、撮影・編集画面に留まっている場合は更新されません。1時間以内の更新を保証するものではありません。

確認APIは公開しているビルドIDだけを返し、認証やDBアクセスは行いません。通常の本番ドメインから最新版へ届く配信構成が前提です。独自のデプロイ固定Cookie（Vercelの `__vdpl` など）や旧版へのルーティングを入れる場合は、確認APIと再読み込みが最新版へ届くことも確認してください。
