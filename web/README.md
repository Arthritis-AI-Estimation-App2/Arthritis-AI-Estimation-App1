...
カメラ撮影後、保存するJPEGをブラウザ内で読み戻し、画面上の手のガイドの内側をチェックします。表示と同じ輪郭データから判定マスクを作り、指間・背景・前腕（viewBoxのy=780以降）を除外します。左右反転と、案内文・余白・縦横比維持による実際の表示位置を保存画像へ反映します。暗さ・明るさ・ぼやけの疑いがあれば「撮り直す」「この画像を使う」を表示します。警告は続行を禁止しません。左右の確認画面にも警告が残り、撮り直し中に「確認に戻る」を選ぶと以前の採用画像を維持します。
