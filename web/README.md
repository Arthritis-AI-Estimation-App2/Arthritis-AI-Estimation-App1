...
カメラ撮影後、保存するJPEGをブラウザ内で読み戻し、画面上の手のガイドの内側をチェックします。表示と同じ輪郭データから判定マスクを作り、指間・背景・前腕（viewBoxのy=780以降）を除外します。左右反転と、案内文・余白・縦横比維持による実際の表示位置を保存画像へ反映します。暗さ・明るさ・ぼやけの疑いがあれば「撮り直す」「この画像を使う」を表示します。警告は続行を禁止しません。左右の確認画面にも警告が残り、撮り直し中に左上の✗を選ぶと以前の採用画像を維持して確認画面へ戻ります。

ファイル選択では手の位置・左右を特定できないため、従来の画像中央に収まる最大の5:8楕円を使います（手形マスクではありません）。ファイルは無変換で保持します。品質結果・計測値は撮影中のメモリだけに保持し、DBやAPIには送信しません。

判定設定の正本は `src/lib/image-quality.ts` の `IMAGE_QUALITY_CONFIG`（`hand-guide-v2`）です。評価画像は拡大せず長辺最大512pxです。手形マスクは輪郭から約8 viewBox単位以上（評価画像で最低2px）を内側へ除外し、位置合わせの小さなずれや境界の補間の影響を減らします。ファイル選択の楕円は従来どおり境界から2px内側を評価します。Laplacianは中心と上下左右がすべて評価範囲内の画素だけで計算します。
...
手形への変更では判定対象と設定バージョンを更新し、数値しきい値は従来の暫定値を維持しています。これらは実写で精度検証済みのしきい値ではありません。ガイドと実際の手がずれて混入した背景の模様、手の大きさ、肌の質感、照明、端末の画像処理の影響を受けます。手の位置検出、ブレとピンボケの区別、ブレに直交する模様が鮮明な場合を含む全方向のブレ検出、局所的な影・反射の網羅的な検出は行いません。

手形対応で検証済み: 5本の指と手の甲のマスク、指間・前腕・境界の除外、背景の変更に対する判定値の不変性、左右反転・縮小・保存画像座標の対応、合成画素での露出不良と縦横のブレ・平滑化、Workerへの手形・左右情報の受け渡し、失敗・中断。

旧楕円方式での検証記録（手形方式のブラウザ実機検証ではありません）: デスクトップChromiumでの合成JPEGによる警告・続行・左右別再撮影・元画像への復帰・未確認・離脱確認。合成カメラ映像で390×844、820×1180、844×390の画面に対するガイド座標と保存画像の対応も確認しています。
